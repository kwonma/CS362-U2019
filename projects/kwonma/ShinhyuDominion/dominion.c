...
int minionCard(int choice2, int choice1, struct gameState *state, int handPos) //BUG choice 1 and 2 are in the wrong order for the parameter list.
{
	int i;
	int j;
	int currentPlayer = whoseTurn(state);
...
int tributeCard(struct gameState *state)
{
	...
	for (i = 0; i <= 2; i ++){
		...
		else if (tributeRevealedCards[i] == estate || tributeRevealedCards[i] == estate || tributeRevealedCards[i] == province || tributeRevealedCards[i] == gardens || tributeRevealedCards[i] == great_hall){//Victory Card Found, BUG duchy is changed to estate.
			drawCard(currentPlayer, state);
			drawCard(currentPlayer, state); 
		}
		else{//Action Card
...
int mineCard(int choice1, int choice2, struct gameState *state, int handPos)
{
	...
	if (choice2 > treasure_map && choice2 < curse)//BUG || changed to &&
	{
		return -1;
	}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
	...
	switch( card ) 
	{
		...
		case baron:
			baronCard(choice1, state, handPos);
